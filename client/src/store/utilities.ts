// From https://github.com/paulkoerbitz/typeful-redux
// WIP, experimentation
...
export class StoreBuilder<STORE_STATE = {}, STORE_DISPATCH = DEFAULT_STORE_DISPATCH> {
    ...
    Intersect<STORE_STATE, {[reducerNameFromType in REDUCER_NAME]: REDUCER_STATE }>,
    Intersect<STORE_DISPATCH, {[reducerNameFromType in REDUCER_NAME]: REDUCER_DISPATCH }>
    > {
    ...
    private make_reducer(prefix: string): any {
        ...
        (reducer as any).register_action = <T>(type: string, handler: any) => {
            ...
            return StoreBuilder.make_action_creator<T>(fulltype, handler.length);
        };
    ...
    public buildReducers(): { reducers: any, dispatchFunctionsFactory: any } {
        let reducers: any = {};
        ...
        for (const prefix in this.reducerBuilders) {
            ...
            for (const actionName in handlers) {
                ...
                actionCreators[prefix][actionName] = actionCreator;
            }
        }
